...
vsi_status vnn_PreProcessPixels
    (
        vsi_nn_graph_t *graph,
        void *img_data,
        uint32_t size
...
//   _load_input_meta();
  memset(&meta, 0, sizeof(vnn_input_meta_t));
//   __android_log_print(ANDROID_LOG_INFO, "libaliJNA", "vsi_nn_GetTensor, graph=%p", graph);
//   __android_log_print(ANDROID_LOG_INFO, "libaliJNA", "vsi_nn_GetTensor, s=%p", graph->input);
  tensor = vsi_nn_GetTensor( graph, graph->input.tensors[0] );
//   __android_log_print(ANDROID_LOG_INFO, "libaliJNA", "vsi_nn_GetTensor end");
