...
AbsEnginePtr DeviceEngineFactory::GetDeviceInference(const std::string name,
...
AbsEnginePtr DeviceEngineFactory::SelectEnginebyType(
  const std::string name,
  std::unordered_map<std::string, AbsEnginePtr> *engine_map) {
  auto iter = engine_map->find(name);
  if (iter == engine_map->end()) {
    //    LOG(ERROR) << "DeviceEngineFactory can't find inference_creator_ KEY:
    //    " << iter->first;
    return nullptr;
  }
  return iter->second;
