...
static void _load_input_meta() {
  ...
  input_meta_tab[0].image.scale = 1.0 / 255.0;
}
...
static vsi_enum _get_file_type(const char *file_name) {
  ...
  if (strcmp(buff, "jpg") == 0 || strcmp(buff, "jpeg") == 0 ||
      strcmp(buff, "JPG") == 0 || strcmp(buff, "JPEG") == 0) {
    type = NN_FILE_JPG;
...
static void _get_jpeg_size(const char *name, uint32_t *bmpWidth,
                           uint32_t *bmpHeight, uint32_t *channel) {
  FILE *jpgFile = NULL;
...
static vsi_status _jpeg_to_bmp(FILE *inputFile, unsigned char *bmpData,
...
static uint8_t *_float32_to_dtype(float *fdata, vsi_nn_tensor_t *tensor) {
  ...
  for (i = 0; i < sz; i++) {
    status =
      vsi_nn_Float32ToDtype(fdata[i], &data[stride * i], &tensor->attr.dtype);
...
static uint8_t *_get_qtensor_data(vsi_nn_tensor_t *tensor, const char *name) {
  ...
  for (i = 0; i < sz; i++) {
    if (fscanf(tensorFile, "%f ", &fval) != 1) {
      ...
      printf(
        "Please check file lines or if the file contains illegal characters\n");
      goto error;
    }
    if (1 == stride) {
      ...
      if (VSI_NN_TYPE_INT16 == tensor->attr.dtype.vx_type) {
        ...
        memcpy(tensorData + i * stride, &int16_temp_value,
               stride * sizeof(uint8_t));
      } else {
        uint16_temp_value = (uint16_t)fval;
        memcpy(tensorData + i * stride, &uint16_temp_value,
               stride * sizeof(uint8_t));
      }
    } else {
...
static uint8_t *_get_tensor_data(vsi_nn_tensor_t *tensor, const char *name) {
  ...
  for (i = 0; i < sz; i++) {
    ...
    status =
      vsi_nn_Float32ToDtype(fval, &tensorData[stride * i], &tensor->attr.dtype);
...
static uint8_t *_get_jpeg_data(vsi_nn_tensor_t *tensor, vnn_input_meta_t *meta,
                               const char *filename) {
  uint32_t i;
...
static void _get_image_handle_buffer(uint32_t width, uint32_t height,
                                     uint32_t channels,
...
static void _get_jpeg_image_align(const char *name, uint32_t width,
...
static vsi_status _handle_multiple_inputs(vsi_nn_graph_t *graph, uint32_t idx,
  ...
  if (fileType == NN_FILE_JPG) {
...

    {
      ...
      _prepare_imageprocess_params(&imageprocess, crop_start, crop_length,
                                   resize_length);
      _get_jpeg_image_align(image_name, attr.size[0], attr.size[1],
                            attr.size[2]);
      status = vsi_nn_op_imageprocess_single_node(
        graph, &attr, &imageprocess, buffer_img_align_addr, tensor);
    }
  } else {
      ...
      if (buffer) {
        ...
        for (i = 0; i < item_ount && !fail_to_read; i++) {
          if (fscanf(fp, "%f ", (float *)&buffer[stride * i]) != 1) {
            printf("Read tensor file fail.\n");
            printf(
              "Please check file lines or if the file contains illegal "
              "characters\n");
            free(buffer);
            fail_to_read = TRUE;
            read_size = 0;
