...
#define NET_TOTAL_TENSOR_NUM \
  (NET_NORM_TENSOR_NUM + NET_CONST_TENSOR_NUM + NET_VIRTUAL_TENSOR_NUM)
...
vsi_nn_graph_t *vnn_CreateBestSim(
  ...
  vsi_nn_SetGraphVersion(graph, VNN_VERSION_MAJOR, VNN_VERSION_MINOR,
                         VNN_VERSION_PATCH);
  ...
  /* @attach_Concat_Concat_309/out0_0:out0 */
  attr.size[0] = 333396;
  attr.size[1] = 1;
  attr.dim_num = 2;
  attr.dtype.scale = 0.13648085296154022;
  ...
  if (enable_pre_post_process) {
    if (pre_process_map_count > 0) {
      for (i = 0; i < pre_process_map_count; i++) {
        status = vsi_nn_AddGraphPreProcess(
          graph, pre_process_map[i].graph_input_idx,
    ...
    if (post_process_map_count > 0) {
      for (i = 0; i < post_process_map_count; i++) {
        status = vsi_nn_AddGraphPostProcess(
          graph, post_process_map[i].graph_output_idx,
