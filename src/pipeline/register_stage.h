...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<DecoratorStage>> inference_creator_;
};
...
public:
  DeviceInferenceRegister(const std::string &name_inference,
                          std::shared_ptr<DecoratorStage> device_inferencer) {
    DeviceInferenceFactory::GetInstance().Register(name_inference, device_inferencer);
  }
